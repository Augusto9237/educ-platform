...
export function ProfileAdmin() {
    const { user, loadingUser } = useContext(AdminContext)
        ...
        <Popover.Root>
                ...
                {!loadingUser && (
                    <>
                        <div className="flex flex-row flex-1 justify-end items-center overflow-hidden gap-2">
                            <strong className="text-base leading-none">{user?.teacher?.name}</strong>
                            <StudentAvatar width="30px" height="30px" url={user?.teacher?.avatarURL} />
                        </div>
                    </>
                )}
            </Popover.Trigger>
            ...
            <Popover.Portal>
                <Popover.Content className="flex flex-col gap-2 min-w-[85px] py-2 rounded bg-backgroundColor-100 shadow-lg ">
                    {!loadingUser && (
                        <Link href='/profileAdmin' className="flex flex-col px-2">
