...
export function FormEditAssessments({ idSubsriber, month, grades, nameSubsriber }: FormAssessmentsPros) {
                    ...
                    <Dialog.Content className='absolute p-4 bg-backgroundColor-100 rounded-2xl xl:max-w-[700px] max-md:w-11/12 w-full  max-w-md top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 overflow-hidden'>
                        ...
                        <form onSubmit={handleSubmitCreateGrades} className='flex flex-col'>
                            <div className='relative flex flex-1 w-full gap-4 py-2'>
                                <select required name="IdSubsriber" id="subSelect" onChange={handleChange} defaultValue={formValues.IdSubsriber} className='p-1 w-full bg-backgroundColor-300 rounded-md'>
                                ...
                                <div className='flex gap-2 max-w-fit items-center'>
                                    ...
                                    <input required type='date' value={formValues.month} name='month' onChange={handleChange} className='bg-backgroundColor-300 rounded-md w-28 p-1' />
                                </div>
                                ...
                                <div key={week.Week.id} className='relative flex flex-col gap-2 py-2'>
                                    ...
                                    <div className='grid grid-cols-5'>
                                        <div className='relative flex gap-2 items-center justify-center'>
                                            ...
                                            <input id={`primaryReview${i + 1}Week`} max={1000} min={0} type='number' value={formValues.weeks[i].Week.primaryReview} name="primaryReview" onChange={handleChange} className='bg-backgroundColor-300 rounded-md p-1 max-w-[55px]' />
                                            <span className='absolute right-0'>+</span>
                                        </div>

                                        <div className='relative flex gap-2 items-center justify-center'>
                                            <label htmlFor="secondReview">2ª Av</label>
                                            <input id={`secondReview${i + 1}Week`} max={1000} min={0} type='number' value={formValues.weeks[i].Week.secondReview}  name="secondReview" onChange={handleChange} className='bg-backgroundColor-300 rounded-md p-1 max-w-[55px]' />
                                            <span className='absolute right-0'>+</span>
                                        </div>

                                        <div className='relative flex gap-2 items-center justify-center'>
                                            <label htmlFor="thirdReview">3ª Av</label>
                                            <input id={`thirdReview${i + 1}Week`} max={1000} min={0} type='number' value={formValues.weeks[i].Week.thirdReview}  name='thirdReview' onChange={handleChange} className='bg-backgroundColor-300 rounded-md p-1 max-w-[55px]' />
                                            <span className='absolute right-0'>+</span>
                                        </div>

                                        <div className='relative flex gap-2 items-center justify-center'>
                                            <label htmlFor="fourthReview">4ª Av</label>
                                            <input id={`fouthReview${i + 1}Week`} max={1000} min={0} type='number' name='fourthReview' value={formValues.weeks[i].Week.fourthReview}  onChange={handleChange} className='bg-backgroundColor-300 rounded-md p-1 max-w-[55px]' />
